...
function Change() {
  ...
  return (
    <div className='profileEl'>
      <h2 style={{ color: 'tomato', marginTop: '25px' }}>Поменяйте что необходимо</h2>
      <form onSubmit={changeInfoHandler}>
        <div className='profileEl' style={{ flexDirection: 'column' }}>
          ...
          <div style={{ minWidth: "400px", margin: "0 auto" }}><label>Изменить описание о себе :
            <textarea value={about} onChange={(e) => setAbout(e.target.value)}
              placeholder="Изменить информацию о себе"
            />
            </label>
            <div><button type='submit'>Записать</button></div>
