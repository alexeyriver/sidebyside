...
import {fetchFindAllJourneyAC} from '../../redux/actions'
...
console.log(stateofAll);
  ...
  const HandlerChanger = (e) => {
    ...
    fetch(`https://geocode-maps.yandex.ru/1.x/?apikey=de443bec-303e-4052-bc88-4e6872551ce0&format=json&geocode=${town}`)
      .then((resp) => resp.json())
      .then((data) => { if (data.response?.GeoObjectCollection.featureMember[0]?.GeoObject.Point.pos) { setDataFetch(data.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(' ')) } else setFlagMapSearch(true)});
    setDataFetch('');
  };
